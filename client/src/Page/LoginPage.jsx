  ...
  return (
    <div className="login-page bg-[url('src/images/login.jpeg')] bg-no-repeat bg-cover bg-center bg-fixed flex items-center justify-center h-screen w-[200vh]">
      {isRegistering ? (
        <div className="bg-black bg-opacity-70 p-5 rounded text-center text-white w-[300px] mx-auto">
          <h2>Sign Up</h2>
          <input type="text" className='w-full p-2.5 my-2 border-none rounded bg-white' placeholder="Username" value={username} onChange={(e) => setUsername(e.target.value)} />
          <input type="email" className='w-full p-2.5 my-2 border-none rounded bg-white' placeholder="Email" value={email} onChange={(e) => setEmail(e.target.value)} />
          <input type="password" className='w-full p-2.5 my-2 border-none rounded bg-white' placeholder="Password" value={password} onChange={(e) => setPassword(e.target.value)} />
          <button className="bg-[#a5e100] text-black p-2.5 rounded cursor-pointer font-bold hover:bg-[#89c700]" onClick={handleRegister}>Register</button>
          <p onClick={() => setIsRegistering(false)}>Already have an account? <span className="text-[#a5e100] cursor-pointer">Login</span></p>
        </div>
      ) : (
        <div className="bg-black bg-opacity-70 p-5 rounded text-center text-white w-[300px] mx-auto">
          ...
          <input type="email" className='w-full p-2.5 my-2 border-none rounded bg-white' placeholder="Email" value={email} onChange={(e) => setEmail(e.target.value)} />
          <input type="password" className='w-full p-2.5 my-2 border-none rounded bg-white' placeholder="Password" value={password} onChange={(e) => setPassword(e.target.value)} />
          <button className="bg-[#a5e100] text-black p-2.5 rounded cursor-pointer font-bold hover:bg-[#89c700]" onClick={handleLogin}>Login</button>
          <p onClick={() => setIsRegistering(true)}>Need an Account? <span className="text-[#a5e100] cursor-pointer">Sign Up</span></p>
        </div>
      )}
      {error && <p className="error-message">{error}</p>}
